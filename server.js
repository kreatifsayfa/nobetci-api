...
app.get('/api/nobetci/cities', async (req, res) => {
  try {
    ...
    res.status(200).json({ ok: false, error: e.message, cities: [], count: 0, fetchedAt: new Date().toISOString() });
  }
});
...
app.get('/api/nobetci/:city', async (req, res) => {
  try {
    ...
    res.status(200).json({
      ok: false,
      citySlug: String(req.params.city || '').toLowerCase().trim(),
      error: e.message,
      stale: true,
      fromCacheFallback: true,
      pharmacies: [],
...
app.get('/api/nobetci-all', async (req, res) => {
  try {
    ...
    res.status(200).json({ ok: false, error: e.message, results: [], cityCount: 0, totalPharmacies: 0, fetchedAt: new Date().toISOString() });
  }
});
